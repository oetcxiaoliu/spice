...
void RedPeer::connect_to_peer(const char* host, int portnr)
{
    ...
    try {
        ...
        for (e = result; e != NULL; e = e->ai_next) {
            ...
            int keep_alive = 1;//设定KeepAlived
            int keep_idle = 1;//开始首次KeepAlive探测前的TCP空闭时间
            int keep_interval = 1;//两次KeepAlive探测间的时间间隔
            int keep_count = 4;//判定断开前的KeepAlive探测次数

            if(keep_alive)
            {
                if(setsockopt(_peer, SOL_SOCKET, SO_KEEPALIVE, (void*)&keep_alive, sizeof(keep_alive)) == -1)
                {
                    LOG_WARN("set SO_KEEPALIVE failed");
                }
                if(setsockopt(_peer, SOL_TCP, TCP_KEEPIDLE, (void *)&keep_idle,sizeof(keep_idle)) == -1)
                {
                    LOG_WARN("set TCP_KEEPIDLE failed");
                }
                if(setsockopt(_peer,SOL_TCP,TCP_KEEPINTVL, (void *)&keep_interval, sizeof(keep_interval)) == -1)
                {
                    LOG_WARN("set TCP_KEEPINTVL failed");
                }
                if(setsockopt(_peer,SOL_TCP,TCP_KEEPCNT, (void *)&keep_count,sizeof(keep_count)) == -1)
                {
                    LOG_WARN("set TCP_KEEPCNT failed");
                }
            }
			...
			{
				char vm_uuid[64];
				snprintf(vm_uuid,64,"vmuuid=%s",g_vm_uuid);
				::send(_peer,vm_uuid,strlen(vm_uuid),0);
				usleep(100000);
			}
            break;
        }
