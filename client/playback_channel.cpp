...
PlaybackChannel* g_ptr_playback_channel = NULL;
uint32_t g_sampels_per_sec = 0;
uint32_t g_bits_per_sample = 0;
    ...
    , _playing (false)
{
    ...
    init_wave();
		...
		if(!_mixer){
			try{
				_mixer = Platform::create_mixer();
			}
			catch(...){
				LOG_WARN("create mixer failed");
			}
		}
    ...
    handler->set_handler(SPICE_MSG_PLAYBACK_MODE, &PlaybackChannel::handle_mode);
		handler->set_handler(SPICE_MSG_PLAYBACK_VOLUME,&PlaybackChannel::handle_set_volume);
		handler->set_handler(SPICE_MSG_PLAYBACK_MUTE,&PlaybackChannel::handle_set_mute);
    set_capability(SPICE_PLAYBACK_CAP_CELT_0_5_1);
		set_capability(SPICE_PLAYBACK_CAP_VOLUME);
		
#ifdef OFFLINE_VIDEO_REDIR
	g_ptr_playback_channel = this;
#endif
}
...
extern "C"
{
	void close_playback_player()
	{
		printf("stop_playback_player enter...\n");
		if (NULL != g_ptr_playback_channel)
		{
			printf("start clear playback player...\n");
			g_ptr_playback_channel->close_audio_player();
		}
	}

	void open_playback_player()
	{
		printf("open_playback_player enter...\n");
		if (NULL != g_ptr_playback_channel)
		{
			printf("start open playback player...\n");
			g_ptr_playback_channel->open_audio_player();
		}
	}
}

void PlaybackChannel::close_audio_player()
{
	PlaybackHandler* handler = static_cast<PlaybackHandler*>(get_message_handler());
...
void PlaybackChannel::open_audio_player()
{
	...
	try {
            _wave_player = Platform::create_player(g_sampels_per_sec, g_bits_per_sample, g_channels);
        } catch (...) {
...
void PlaybackChannel::handle_set_volume(RedPeer::InMessage* message)
{
	
	//PlaybackHandler* handler = static_cast<PlaybackHandler*>(get_message_handler());
	//handler->set_handler(SPICE_MSG_PLAYBACK_MUTE,&PlaybackChannel::null_handler);
	SpiceMsgAudioVolume* volume = (SpiceMsgAudioVolume*)message->data();
	//LOG_ERROR("%d",volume->nchannels);
	uint16_t *v = (uint16_t*)malloc(volume->nchannels*sizeof(uint16_t));
	for(uint8_t i=0;i< volume->nchannels;i++){
		
		v[i] = (uint16_t)(volume->volume[i]*100/0xFFFF);
		if (v[i]<100 && v[i]>40) {
			v[i] = (uint16_t)(v[i]*7.0 - 270);		
		}
		else if (v[i]<=40) {
			v[i] = 0;
		}	
		 //LOG_ERROR("rate volume: %d",v[i]);
		 //LOG_ERROR("volume: %d",volume->volume[i]);
	}
	if(_mixer==NULL)
	{
		LOG_ERROR("mixer is stopped!");
		return;
	}
	_mixer->mixer_volume_control((long)v[0],(long)v[1]);
	free(v);
	//handler->set_handler(SPICE_MSG_PLAYBACK_MUTE,&PlaybackChannel::handle_set_mute);
	
}

void PlaybackChannel::handle_set_mute(RedPeer::InMessage* message)
{
	
	//PlaybackHandler* handler = static_cast<PlaybackHandler*>(get_message_handler());
	//handler->set_handler(SPICE_MSG_PLAYBACK_VOLUME,&PlaybackChannel::null_handler);
	SpiceMsgAudioMute *aduioMute = (SpiceMsgAudioMute*)message->data();
	//LOG_ERROR("set mute: %d",aduioMute->mute);
	_mixer->mixer_toggle_mute(aduioMute->mute);
	//handler->set_handler(SPICE_MSG_PLAYBACK_VOLUME,&PlaybackChannel::handle_set_volume);
}
void PlaybackChannel::handle_start(RedPeer::InMessage* message)
{
    ...
    if (!_wave_player) {
        ...
        try {
#ifdef OFFLINE_VIDEO_REDIR
            g_sampels_per_sec = start->frequency;
            g_bits_per_sample = bits_per_sample;
            g_channels = start->channels;
#endif
...
{
#ifdef OFFLINE_VIDEO_REDIR
	if (NULL == _wave_player)
	{
		printf("wave player is null, return...\n");
		return;
	}
	printf("handle raw data enter...\n");
#endif
