...
/* Code by neo. Apr 7, 2013 17:31 */
RedScreen::RedScreen(Application& owner, int id, const std::string& name, const std::string& icon, int width, int height)
    : _owner (owner)
    , _id (id)
    , _refs (1)
    , _window (*this)
...
bool RedScreen::IsMinimized()
{
	return _window.is_iconic();
}
...
/* Code by neo. Apr 7, 2013 17:33 */
void RedScreen::set_icon(const std::string& icon)
{
	if (!icon.empty()) {
		string_printf(_icon, icon.c_str());
	}
	_window.set_icon(icon);
}
...
void RedScreen::save_position()
{
	...
	LOG_INFO("save_pos[%d, %d]", _save_pos.x, _save_pos.y);
}
...
void RedScreen::__show_full_screen()
{
	...
	LOG_INFO("window.move_and_resize(pos.x: %d, pos.y:%d, mon.x:%d, mon.y:%d)", position.x, position.y, monitor_size.x, monitor_size.y);
    ...
    if (!(_out_of_sync = _monitor->is_out_of_sync())) {
        ...
        if (monitor_size.x >= _size.x){
            _origin.x = (monitor_size.x - _size.x) / 2;
        }else{
            _origin.x = 0;
        }
        if (monitor_size.y >= _size.y) {
            _origin.y = (monitor_size.y - _size.y) / 2;
        }else{
            _origin.y = 0;
        }
	...
	LOG_INFO("window.set_origin(origin.x:%d, origin.y:%d)", _origin.x, _origin.y);
...
{
    RecurciveLock lock(_update_lock);
	LOG_INFO("window.show(screen_id: %d)", _monitor ? _monitor->get_screen_id() : 0);
