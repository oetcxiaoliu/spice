...
void AgentTimer::response(AbstractProcessLoop& events_loop)
{
	...
	_client->set_agent_mon_config_sent(false);
	_client->set_agent_guest_config_sent(false);
...
RedClient* RedClient::_singleton = NULL;


RedClient* RedClient::get(Application& application)
{
    if(!_singleton)
    {
        _singleton = new RedClient(application);
    }
    return _singleton;
}

/* Code by neo. Jun 20, 2014 16:40:37 */
RedClient::RedClient(Application& application)
    ...
    , _agent_guest_config_sent (false)
    , _agent_autologon_sent(false)
    , _agent_autologon_sent_count(0)
...
#ifdef USE_DEVREDIR
    devredirPrinter = new DevredirPrinter();
    devredirCam     = DelVideoMessage::getInstance();
    guestOSAutoLogon = NULL;
#endif
    pGuestOSAutoLogonInfo.iGuestOSAutoLogon = 0;
    strcpy(pGuestOSAutoLogonInfo.szGuestOSAutoLogonUsername, "");
    strcpy(pGuestOSAutoLogonInfo.szGuestOSAutoLogonPassword, "");
    strcpy(pGuestOSAutoLogonInfo.szGuestOSAutoLogonDomain, "");

    _current_screen_resolution.width = 0;
    _current_screen_resolution.height = 0;
    get_current_resolution();

    start();
}

InputState RedClient::get_input_state()
{
    Lock lock(_application._input_lock);
    return _application._input_state;
}


ControlState RedClient::get_control_state()
{
    return _application._control_state;
}
void RedClient::set_operation_timer(int minutes)
{
    _operation_timer = minutes;
}

int RedClient::get_operation_timer()
{
    return _operation_timer;
}
...
#ifdef USE_DEVREDIR
    if (devredirPrinter != NULL) delete devredirPrinter;
    if (devredirCam != NULL) delete devredirCam;
#endif
}

#ifdef USE_DEVREDIR
void RedClient::send_msg_to_device(uint32_t msg_source_flag, uint32_t msg_type, int opaque,uint8_t *data, uint32_t data_size)
{
    Message* message = new Message(SPICE_MSGC_MAIN_AGENT_DATA);
    VDAgentMessage* msg = (VDAgentMessage*)
        spice_marshaller_reserve_space(message->marshaller(), sizeof(VDAgentMessage) + data_size);
    msg->protocol = VD_AGENT_PROTOCOL;
    msg->type = msg_type;
    msg->msg_source_flag = msg_source_flag;
    msg->opaque = opaque;
    msg->size = data_size;
...
void RedClient::get_current_resolution()
{
    ...
    _current_screen_resolution.width = width;
    _current_screen_resolution.height = height;
}

/* Code by neo. Jun 20, 2014 16:47:27 */
bool RedClient::abort()
{
#ifdef USE_DEVREDIR
    if (guestOSAutoLogon != NULL)
    {
        delete guestOSAutoLogon;
        guestOSAutoLogon = NULL;
    }
#endif
    if (!_aborting) {
...
ChannelFactory* RedClient::find_factory(uint32_t type)
{
    ...
    printf("%s: No factory for %d. \n", __FUNCTION__, type);

	return NULL;
}

/* Code by neo. Apr 24, 2013 15:46 */
void RedClient::create_channel(uint32_t type, uint32_t id)
{
    ...
    {
    	printf("%s: Create usb device redir channel.", __FUNCTION__);
        UsbDeviceManager::getInstance()->spice_usb_device_manager_register_channels(channel);
    }
#endif
    ...
    if (type == SPICE_CHANNEL_USBREDIR)
    {
        UsbDeviceManager::getInstance()->spice_usb_device_manager_new_channel_cb(channel);
    }
    else
    {
#endif
...
void RedClient::send_agent_monitors_config()
{
    ...
    for (iter = monitors.begin(), i = 0; iter != monitors.end(); iter++, i++) {
        ...
        /*fixed bug when os starting client not best resolution*/
        if(i == 0 && _current_screen_resolution.width != 0
            && _current_screen_resolution.height !=0) {
            mon_config->monitors[0].width = _current_screen_resolution.width;
            mon_config->monitors[0].height = _current_screen_resolution.height;
        }
    }
    ASSERT(_agent_tokens)
    _agent_tokens--;
...
void RedClient::send_agent_announce_capabilities(bool request)
{
    ...
    VD_AGENT_SET_CAPABILITY(caps->caps, VD_AGENT_CAP_GUEST_AUTOLOGON);
    ASSERT(_agent_tokens)
    _agent_tokens--;
    post_message(message);
}
...
void RedClient::send_agent_guest_config(int flags)
{
	if (!VD_AGENT_HAS_CAPABILITY(_agent_caps, _agent_caps_size,
		VD_AGENT_CAP_GUEST_AUTOLOGON))
		return;

	if(pGuestOSAutoLogonInfo.iGuestOSAutoLogon == 0) {
		return ;
	}
	...
	VDAgentMessage* msg = (VDAgentMessage*)
		spice_marshaller_reserve_space(message->marshaller(), sizeof(VDAgentMessage));
	VDAgentSimpleLogon* logon_config;

	DBG(0,"");
	msg->protocol = VD_AGENT_PROTOCOL;
	msg->type = VD_AGENT_GUEST_CONFIG;
	msg->opaque = 0;
	msg->size = sizeof(VDAgentSimpleLogon);

	logon_config = (VDAgentSimpleLogon*)
		spice_marshaller_reserve_space(message->marshaller(), sizeof(VDAgentSimpleLogon));

	logon_config->flags = flags;
	strcpy(logon_config->user, pGuestOSAutoLogonInfo.szGuestOSAutoLogonUsername);
	strcpy(logon_config->passwd, pGuestOSAutoLogonInfo.szGuestOSAutoLogonPassword);
	if(strlen(pGuestOSAutoLogonInfo.szGuestOSAutoLogonDomain) == 0){
		strcpy(logon_config->domain, "");
	}else{
		strcpy(logon_config->domain, pGuestOSAutoLogonInfo.szGuestOSAutoLogonDomain);
	}

	ASSERT(_agent_tokens)
		_agent_tokens--;
	post_message(message);
	_agent_guest_config_sent = true;
	_agent_reply_wait_type = VD_AGENT_GUEST_CONFIG;
}
...
/* Code by neo. Jun 20, 2014 16:49:1 */
...
#ifdef USE_DEVREDIR
    guestOSAutoLogon = new GuestOSAutoLogon(this);
#endif
}

/* Code by neo. Apr 24, 2013 15:53 */
...
#define  _CREATE_DISPCHANNEL_FIRST_
    //create Display channel first

#ifdef _CREATE_DISPCHANNEL_FIRST_
    for (unsigned int i = 0; i < init->num_of_channels; i++) {
        if (channels[i].type == SPICE_CHANNEL_DISPLAY )
        {
    ...
    for (unsigned int i = 0; i < init->num_of_channels; i++) {
		...
		printf("%s: channels[i].type= %d, channels[i].id= %d \n", __FUNCTION__, channels[i].type, channels[i].id);
...
#ifdef USE_USBREDIR
    UsbDeviceManager::getInstance()->spice_usb_device_manager_start();
    //DelVideoMessage::getInstance()-> AddUsbVideodevices();
#endif
}
    ...
    VDAgentAnnounceCapabilities* caps, uint32_t msg_size)
{
    ...
    if (caps->request) {
        ...
        // autologon, starting OS system, need to sent 2 autologon message
        if(!_agent_autologon_sent && _agent_autologon_sent_count < 3) {
            send_agent_guest_config(1);
            sleep(1.8);
            _agent_autologon_sent_count++;
        }

        if(_agent_autologon_sent_count == 2) {
            _agent_autologon_sent = true;
        }
    }else{
        // autologon, OS system has working, need to sent 1 autologon msg
        _agent_autologon_sent = true;
        send_agent_guest_config(1);
    }
...
/* Code by neo. Jul 23, 2013 14:26 */
void RedClient::dispatch_agent_message(VDAgentMessage* msg, void* data)
{
#ifdef USE_DEVREDIR
    if ((msg->msg_source_flag == VD_AGENT_MSG_DEVICE_FOR_PRINTER) && 
        (_application._devredir_filter_string.find("printer") == -1))
    {
        devredirPrinter->msg_deal(msg, data);
        return;
    }
    
    if ((msg->msg_source_flag == VD_AGENT_MSG_DEVICE_FOR_VIDEO) && 
        (_application._devredir_filter_string.find("video") == -1) && 
        (devredirCam != NULL))
    {
        devredirCam->VideoMessgDel(msg,data,this);
        return;
    }
