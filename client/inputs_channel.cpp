    ...
    , _operation_timer (NULL)
{
    ...
    int operation_timer = client.get_operation_timer();
    if (operation_timer) {
        _timer_flag = true;
    }
    if (_timer_flag) {
        g_operation_time = Platform::get_monolithic_time() / 1000 / 1000;
        _operation_timer = new Thread(InputsChannel::operation_timer, (void *)&client);
    }
}

InputsChannel::~InputsChannel()
{
    if (_timer_flag && !_operation_timer) {
        delete _operation_timer;
        _operation_timer = NULL;
    }
}
...
{
    if (get_client().get_control_state() == CONTROL_OFF
        || get_client().get_input_state() == INPUT_OFF) {
        return;
    }
...
void* InputsChannel::operation_timer(void *data)
{
    while (1) {
        uint64_t now = Platform::get_monolithic_time() / 1000 / 1000;
        int timeout = (int)(now - g_operation_time);
        RedClient* client = (RedClient *)data;
        int operation_timer = client->get_operation_timer();
        if (timeout >= operation_timer) {
            client->quit();
            break;
        }
        usleep(10000);
