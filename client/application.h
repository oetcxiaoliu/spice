...
extern "C" void video_update_screen();
extern "C" void video_hide_spice_screen();
extern "C" void video_end();
extern "C" void video_get_info(int * width,int * height);
#endif
...
public:
	...
	void set_title_icon(const std::string& icon);
    void hide();
    void show();
    void external_show();
    void connect();
...
private:
    //do response double CONTROL Key
    int connect_host(const char *ip,int port);
    static void *freeview_thread(void *param);
    void handle_freeview_key(RedKey key,bool pressed);
    void notify_freeview_key();
    ...
    void configure_spice_opt(const char *args, bool file = false);
#ifdef USE_GUI
...
private:
    ...
    std::string _title;
    std::string _title_icon;
    std::string _custom_hotkeys;
...
#endif
/* Code by neo. Apr 24, 2013 13:44 */
#ifdef USE_USBREDIR
    UsbDeviceManager *_devMgr;
#endif
#ifdef USE_DEVREDIR
public:
    std::string _devredir_filter_string;
#endif
#ifdef OFFLINE_VIDEO_REDIR
    bool _video_native_redirect;
    VideoRedirRole _video_redirect_role;
#endif
#ifdef ONLINE_VIDEO_REDIR
    bool _video_online_redirect;
#endif
};
