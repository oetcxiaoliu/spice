...
extern bool g_cid_continue;
bool g_cid_return = false;
...
void XEventHandler::on_event()
{
    ...
    if (XFilterEvent(&event, None)) {
        ...
        if(0 == g_keywindow)
        {
            g_keywindow = (Window)event.xany.window;
        }
...
void XPlatform::key_press(int keysm)
{
    ...
    xev.y_root = 1; 
    xev.state = 0;
    xev.keycode= XKeysymToKeycode( x_display, keysm); 
    xev.same_screen = 1; 

    (void) XSendEvent(x_display, xev.window, True, KeyPressMask,(XEvent *)&xev); 
    XFlush(x_display); 

    printf("Key pressed\n");
    printf("++++++Window:%u\n",(unsigned int)platform_win);
}


void XPlatform::key_release(int keysm)
{
    XKeyEvent xev;
    ...
    (void) XSendEvent(x_display, xev.window, True, KeyReleaseMask,(XEvent *)&xev); 
    XFlush(x_display); 
}
...
void Platform::send_toggle_request()
{
    XPlatform::key_press(XK_Shift_L);
    XPlatform::key_press(XK_F11);
    XPlatform::key_press(XK_End);
    XPlatform::key_press(XK_Control_L);

    XPlatform::key_release(XK_Control_L);
    XPlatform::key_release(XK_End);
    XPlatform::key_release(XK_F11);
    XPlatform::key_release(XK_Shift_L);
}
...
MixerAbstract* Platform::create_mixer()
{
    return new Mixer();
}
...
static void mono_cursor(const SpiceCursorHeader &header, const uint8_t *data, uint32_t *cur_data)
{
    const uint8_t *xor1, *and1;
    uint8_t *dest;
    int bpl, x, y, bit;

    bpl = (header.width + 7) / 8;
    and1 = data;
    xor1 = and1 + bpl * header.height;
    dest  = (uint8_t *)cur_data;
    for (y = 0; y < header.height; y++) {
        bit = 0x80;
        for (x = 0; x < header.width; x++, dest += 4) {
            if (and1[x/8] & bit) {
                if (xor1[x/8] & bit) {
                    /* flip -> hmm? */
                    dest[0] = 0x00;
...
XLocalCursor::XLocalCursor(CursorData* cursor_data)
{
    ...
    case SPICE_CURSOR_TYPE_MONO:
		mono_cursor(header, data, cur_data);
