...
#ifdef USE_USBREDIR
	pthread_t thread_id;
	xUsbThread = pthread_create(&thread_id, NULL, usb_hotplug_listening_thread, NULL);
#endif 
}
...
Window RedWindow::get_window()
{
    return _win;
}


bool RedWindow::is_iconic()
{
	/*
	int ret = XIconifyWindow(x_display,_win, _screen);
	if(!ret){
		return true;
	}
	...
	*/
	...
	if(!XGetWindowAttributes(x_display, _win, &attr))
	{	
		THROW("XGetWindowAttrubute error.");
	}
...
/* Code by neo. Apr 7, 2013 9:52 */
void RedWindow::set_icon(const std::string& icon)
{	
	XWMHints* win_hints;
	Pixmap icon_pixmap;
	...
	XReadBitmapFile(x_display, _win, icon.c_str(), &bitmap_width, &bitmap_height, &icon_pixmap, &x, &y);
	...
	XSetWMHints(x_display, _win, win_hints);
	XFree(win_hints);
}
...
/*
static int WaitForNotify(Display *dpy, XEvent *e, char *arg)
{
...
void RedWindow::activate()
{
	...
	//XEvent event;
    //XIfEvent(x_display, &event,WaitForNotify, (char*)_win);
...
/* Code by neo. May 3, 2013 14:53 */
#ifdef USE_USBREDIR
int RedWindow_p::init_hotplug_sock(void)
{
...
void *RedWindow_p::usb_hotplug_listening_thread(void *host_thread_parameter)
{
	//int hotplug_sock = init_hotplug_sock();
	int hotplug_sock = g_kernel_udev_sock;
	int sum, size;
	char *str;
	...
	while(1)
	{
		...
		if (strstr(buf, "ACTION=add") && 
			strstr(buf, "pci") && 
			strstr(buf, "SUBSYSTEM=usb") && 
			strstr(buf, "DEVTYPE=usb_device") &&
			strstr(buf, "BUSNUM"))
		{
			printf("%s: usb device insert. \n", __FUNCTION__);
			sleep(1);
			UsbDeviceManager::getInstance()->spice_usb_device_manager_device_event(USB_DEVICE_EVENT_ADD, iProd, iVend);
		...
		if (strstr(buf, "ACTION=remove") && 
			...
			strstr(buf, "BUSNUM"))
		{
			printf("%s: usb device remove. \n", __FUNCTION__);
			
			char *pStart = strstr(buf, "PRODUCT");
			char *pEnd;
			
			pStart = strstr(pStart, "=") + 1;
			pEnd = strstr(pStart, "/") - 1;
			memcpy(sVend, pStart, pEnd - pStart + 1);
			iVend = (uint16_t)(htoi(sVend));
			
			pStart = NULL;
			pStart = pEnd + 2;
			pEnd = NULL;
			pEnd = strstr(pStart, "/") - 1;
			memcpy(sProd, pStart, pEnd - pStart + 1);
			iProd = (uint16_t)(htoi(sProd));
			
			UsbDeviceManager::getInstance()->spice_usb_device_manager_device_event(USB_DEVICE_EVENT_REMOVE, iProd, iVend);

#ifdef  USE_DEVREDIR
			DelVideoMessage::getInstance()->RemoveUsbVideoDevice(iProd,iVend);
#endif
		}
	}
