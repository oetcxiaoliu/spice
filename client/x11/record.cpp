                        ...
                        uint32_t channels)
{
    ...
    const char* pcm_device = "default"; // "default" ???
    ...
    if ((err = snd_pcm_hw_params_set_rate_near(_pcm, _hw_params, &sampels_per_sec, 0)) < 0) {
        char sample[256]={0};
        sprintf(sample,"%d",sampels_per_sec);
        LOG_ERROR("cannot set sample rate %s", sample);
        return false;
    }
